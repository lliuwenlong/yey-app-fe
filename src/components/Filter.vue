<template>
    <div class="Filter">
        ...
        <div
            v-for="(v,i) in types"
            :key="i"
            v-show="!((userInfo.roleStatus == 4||userInfo.roleStatus == 7||userInfo.roleStatus == 5)&&(v=='park'||v=='class'))"
        >
        ...
        <div v-if="month||day||times||week">
            <van-row style="background:white">
                ...
                <van-col
                    span="1"
                    v-if="times"
                    style="line-height: 44px; text-align: center;background: white;"
                >~</van-col>
                <van-col span="9" v-if="times">
                    <van-field
                        @click="timesF2 = true"
...
export default {
            ...
            config: {
                ...
                class: () => {
                    return {
                        ...
                        p_id:
                            this.active.park + 1
                                ? this.list.park[this.active.park].id
                                : 0,
                        status: 2,
                        load: false //是否初始加载
                    };
    ...
    methods: {
        ...
        change(key) {
            ...
            if (
                key == "park" &&
                this.types.indexOf("class") != -1 &&
                this.userInfo.roleStatus != 4 &&
                this.userInfo.roleStatus != 5
            )
                this.getList("class");
        ...
        getList(key) {
            console.log(key);
            this.axios
                .post(this.config[key]().path, this.config[key]())
                .then(res => {
                    if (res.data.data && res.data.data.length != -1) {
                        this.list[key] =
                            this.all === true
                                ? [{ id: undefined, name: "全部" }, ...res.data.data]
                                : res.data.data;
                        this.list = { ...this.list };
                        this.active[key] = 0;
                        this.change(key);
    ...
    created() {
        ...
        if (this.timesData[0] && this.timesData[1]) {
            ...
            this.data.times[0] = this.moment(this.currentDate3).format(
                "YYYY-MM-DD"
            );
            this.currentDate4 = new Date(this.timesData[1]);
            this.data.times[1] = this.moment(this.currentDate4).format(
                "YYYY-MM-DD"
            );
            this.emit();
            return;
