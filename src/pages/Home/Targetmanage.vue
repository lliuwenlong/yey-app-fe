...
<template>
    <div class="content">
        ...
        <ul style="margin-top:5px;" v-if="list != null">
            ...
            >
                ...
                <van-row style="padding-bottom:10px;">
                    <van-col span="12">
                        <span
                            style="border-radius:3px;width:20px;height:10px;background-color:#000;display:inline-block;"
                        ></span>
                        指标{{listMsg[i]}}%
                    </van-col>
                    <van-col span="12">
                        <span
                            style="border-radius:3px;width:20px;height:10px;background-color:red;display:inline-block;"
                        ></span>
                        达成率{{listMsg[i]}}%
                    </van-col>
                </van-row>
        ...
        <span
            style="display:block;line-height:200px;font-size:24px;margin-top:5px;width:96%;margin:2%;height:200px;background-color:white;margin-top:10px;text-align:center;"
            v-else
        >暂无数据</span>
    </div>
</template>
...
export default {
    ...
    methods: {
        ...
        getList(types) {
            ...
            this.axios.post("/Wages/app_tong", savePost).then(res => {
                this.list = res.data.data;
                for (const key in this.tabledata1) {
                    let listData = res.data.data[this.tabledata1[key].key]
                        .replace(/%/g, "")
                        .split("@");
                    this.listMsg[key] = listData[1];
                    this.listMag[key] = listData[2];
                    this.options[key].series[0].data = [
                        { value: listData[1] },
                        { value: listData[2] }
                    ];
                    this.options[key].title.subtext =
                        this.tabledata1[key].sex +
                        listData[0] +
                        this.tabledata1[key].radio;
