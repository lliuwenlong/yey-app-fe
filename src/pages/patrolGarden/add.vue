        ...
        <van-tabs v-model="active">
            <van-tab v-for="(v,i) in types" :key="i" :title="v.type">
                <van-cell-group>
                    <van-cell title="图片">
                        <template>
                            <van-uploader :after-read="file=>onRead(file,i)" style="dicplay:inline-block">
                                <img v-if="data[i].img" :src="'/public/img/'+data[i].img" width="50%">
                ...
                <van-cell-group>
                    <van-field v-model="data[i].desc" label="说明" placeholder="键入内容" type="textarea" rows="2" autosize/>
                </van-cell-group>
                <van-cell-group>
                    <van-field v-model="data[i].sug" label="建议" placeholder="键入内容" type="textarea" rows="2" autosize/>
                </van-cell-group>
                <van-cell-group>
                    <van-field v-model="data[i].genjin" label="专项跟进" placeholder="键入内容" type="textarea" rows="2" autosize/>
                </van-cell-group>
                <van-cell-group>
                    <van-field v-model="data[i].jx_zhu" label="教学助理" placeholder="键入内容" type="textarea" rows="2" autosize/>
                </van-cell-group>
            </van-tab>
        </van-tabs>
    </div>
