    ...
    <div>
        ...
        <van-cell-group>
            <div class="header">
                ...
                <span class="sum">应收额：{{incomeData.num}}元</span>
            </div>
        ...
        <van-cell-group>
            <div class="header">
                ...
                <span class="sum">返费额：{{returnData.num}}元</span>
            </div>
        ...
        <van-cell-group>
            <div class="header">
                ...
                <span class="company">金额/(元)</span>
                <span class="sum">合计：{{(+variableFeeData.num).toFixed('2')}}元</span>
            </div>
        ...
        <van-cell-group>
            <div class="header">
                ...
                <span class="company">金额/(元)</span>
                <span class="sum">合计：{{fixedCostData.num}}元</span>
            </div>
...
export default {
    ...
    methods: {
        ...
        getList(data) {
            ...
            this.axios
                .post(
                    "/FinancialStatement/get_Border_Interests",
                    this.getParams()
                )
                .then(res => {
                    ...
                    for (const key in res.data) {
                        if (key.indexOf("占比") != -1) {
                            this.fang[key.split("占比")[0]] = this.fang[
                                key.split("占比")[0]
                            ]
                                ? this.fang[key.split("占比")[0]]
                                : {};
                            this.fang[key.split("占比")[0]].zhanbi =
                                res.data[key];
                        } else {
                            this.fang[key] = this.fang[key]
                                ? this.fang[key]
                                : {};
        ...
        getIncome() {
            ...
            //     });
            this.axios
                .post("/Test/yingshou", { ...this.getParams() })
                .then(res => {
                    this.incomeData = res.data.data ? res.data.data : {};
                    const money = this.incomeData;
                    const keys = Object.keys(money).filter(item => item != 'num');
                    this.incomeOption = {
                        xAxis: {
                            type: "category",
                            data: keys
        ...
        getReturndata() {
            ...
            //         };
            //     });
            this.axios.post("/test/fan", { ...this.getParams() }).then(res => {
                this.returnData = res.data.data ? res.data.data : {};
                const money = this.returnData;
                const keys = Object.keys(money).filter(item => item != 'num');
                this.returnOption = {
                    xAxis: {
                        type: "category",
                    ...
                    series: [
                        {
                            data: keys.map(item => (+money[item]).toFixed("2")),
                            type: "bar",
                            itemStyle: this.itemStyle,
                            barWidth: 20
                        }
        ...
        getFixedCost() {
            ...
            //         };
            //     });
            this.axios.post("/test/gu", { ...this.getParams() }).then(res => {
                this.fixedCostData = res.data.data ? res.data.data : {};
                const money = this.fixedCostData;
                const keys = Object.keys(money).filter(item => item != 'num');
                this.fixedCostOption = {
                    xAxis: {
                        type: "category",
        ...
        getVariableFee() {
            // this.axios
            //     .post("/FinancialStatement/variableFee", {
            //         ...this.getParams()
            //     })
            //     .then(res => {
            ...
            //         };
            //     });
            this.axios.post("/test/bian", { ...this.getParams() }).then(res => {
                this.variableFeeData = res.data.data ? res.data.data : {};
                const money = this.variableFeeData;
                const keys = Object.keys(money).filter(item => item != 'num');
                this.variableFeeOption = {
                    xAxis: {
                        type: "category",
